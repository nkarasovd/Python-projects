...
class RabinKarp:
    def __init__(self, pattern, text):
        ...
        self.hashes = [0] * (self.len_t - self.len_p + 1)
        self._multiplier = 1
        self._prime = 60919
    ...
    def _precompute_hashes(self):
        ...
        y = 1
        for _ in range(1, self.len_p + 1):
            y = (y * self._multiplier) % self._prime
        for i in reversed(range(self.len_t - self.len_p)):
            self.hashes[i] = ((self._multiplier * self.hashes[i + 1]) + ord(self.text[i]) - (
                    y * ord(self.text[i + self.len_p]))) % self._prime
