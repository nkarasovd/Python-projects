    ...
    def preprocess(self):
        ...
        while q:
            ...
            while True:
                ...
                current_ed = len(sc) - len(self.adj[0][v]) - len(self.adj[1][v]) + self.contractedNeighbors[v] + \
                             self.level[v] + nc
...
def make_graph(ch, vertices):
    ...
    for i in range(n):
        for j in range(n):
            ...
            graph[i][j] = l if l != -1 else float("inf")
    return graph
...
def optimal_path(graph):
    class SubProblem():
        def __init__(self, current, distance, processed):
            self.current = current
            self.distance = distance
            self.processed = processed  # set
            self.uid = hash(tuple([current, tuple(processed)]))

        def __eq__(self, other):
            return self.current == other.current and self.processed == other.processed

    n = len(graph)
    # all distances to starting node
    csets = dict()
    for u in range(1, n):
        sub = SubProblem(u, graph[u][0], set([u, 0]))
        csets[sub.uid] = sub

    for _ in range(n - 2):
        newsets = dict()
        for _, sub in csets.items():
            for v in range(n):
                if not v in sub.processed:
                    d = graph[v][sub.current] + sub.distance
                    newsub = SubProblem(v, d, sub.processed | set([v]))
                    if newsub.uid in newsets:
                        if newsets[newsub.uid].distance > d:
                            newsets[newsub.uid] = newsub
                    else:
                        newsets[newsub.uid] = newsub
        csets = newsets
    ...
    minimum = float("inf")
    for _, sub in csets.items():
        minimum = min(minimum, sub.distance + graph[0][sub.current])
